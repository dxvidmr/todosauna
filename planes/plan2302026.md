...
- F8.2 completada (vistas operativas de moderacion + export y runbook).
- F12.1 completada (runbooks operativos).
- F12.2 completada (alertas operativas cleanup).
- F12.3 en progreso (cierre operativo upload pipeline).
- Orden de ejecucion vigente: `12.3 -> 13`.
...
2. Crear `testimonios` con:
   ...
   - `exported_at timestamptz null` (marca manual de exportacin CSV)
...
4. RLS:
   ...
   - slo ejecucin de RPC.
   - exposicin pblica nicamente por vista/RPC de testimonios aprobados y exportados.
...
1. Moderacin:
   - testimonios: `status` + `exported_at`
...
7. Seguridad:
   ...
   - slo vistas/RPC pblicos exponen datos exportados.
8. Regresin:
...
- Testimonios visibles slo con `status='aprobado'` y `exported_at is not null`.
...
| RF-005 | QA automatizada | Anadir pruebas automatizadas para bootstrap, bind de sesion y reset a `unasked`. | Alto | Medio | Fase 7 | completado | Suite E2E cubre bootstrap y reutilizacion de sesion en lectura. |
...
| RF-019 | QA e2e | Crear pruebas end-to-end del flujo primera libre / segunda bloqueada / seleccion de modo. | Alto | Medio | Fase 7 | completado | Suite smoke valida gate de 1ra/2da contribucion con modal de modo. |
...
| RF-023 | Upload pipeline | Sustituir fallback manual de IDs de Drive por subida segura completa (token + finalize/cancel + cleanup). | Alto | Medio | Fase 7 | en progreso | Pipeline staged activo; pendiente evidencia operativa manual completa (exito/cancelacion/cleanup). |
...
| RF-025 | Operacion cleanup | Anadir alertas operativas para `cleanup_failed` y tendencia de huerfanos en staging. | Medio | Bajo | Fase 8 | completado | Workflow falla por metricas degradadas y publica resumen operativo en cada corrida. |
